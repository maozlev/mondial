...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result?.error) {
        setError("ההרשמה הצליחה אך הכניסה נכשלה. אנא היכנס ידנית.");
    ...
    <div className="min-h-screen flex items-center justify-center px-4 py-12" dir="rtl">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400 mt-2">צור חשבון כדי להצטרף לניחושים</p>
        </div>
        ...
        >
          ...
          <div>
            <label className="block text-sm text-gray-400 mb-1">סיסמה (לפחות 8 תווים)</label>
            <input
              type="password"
              value={form.password}
          ...
          <p className="text-center text-sm text-gray-500">
            כבר רשום?{" "}
            <Link href="/api/auth/signin" className="text-yellow-400 hover:text-yellow-300">
              כניסה
            </Link>
          </p>
          ...
          <div className="relative">
            ...
            <div className="relative flex justify-center text-xs">
              <span className="bg-gray-900 px-2 text-gray-500">או</span>
            </div>
          </div>

          <button
            type="button"
            onClick={() => signIn("google", { callbackUrl: "/activate" })}
            className="w-full flex items-center justify-center gap-2 bg-white hover:bg-gray-100 text-gray-900 font-medium py-2.5 rounded-lg transition-colors text-sm"
          >
